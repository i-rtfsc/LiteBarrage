...
public class PermissionImpl implements IPermissionApi {
    ...
    public void initUrgentPermission(final BaseActivity activity) {
        final String[] permissions = {Manifest.permission.READ_PHONE_STATE, Manifest.permission.WRITE_EXTERNAL_STORAGE};
    ...
    @Override
    public boolean listenerNotification(Context activity, String className) {
        ...
        if (notifications == null) {
            ...
            if (!notifications.contains(className)) {
                Intent intent = new Intent(Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS);
                activity.startActivity(intent);
                return false;
            } else {
